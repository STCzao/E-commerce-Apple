  ...
  return (
    <div className="bg-[#f5f5f7] min-h-screen">
      ...
      <div className="pt-24 pb-0 px-6 md:px-16 max-w-7xl mx-auto">
        <nav aria-label="Miga de pan" className="flex items-center gap-2 text-xs text-[#6e6e73]">
          <Link to="/catalogo" className="hover:text-[#1d1d1f] transition-colors flex items-center gap-1">
          ...
          <span className="text-[#1d1d1f] font-medium truncate max-w-[200px]">{name}</span>
        </nav>
      </div>
      ...
      <div className="px-6 md:px-16 py-8 max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
            ...
            <div className="relative aspect-square md:aspect-[4/3] bg-white rounded-3xl border border-black/[0.06] overflow-hidden shadow-sm">
              <AnimatePresence mode="wait">
                {images.length > 0 ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-black/15" aria-hidden="true">
          ...
          <div className="lg:w-[45%] flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3 border-t border-black/[0.06] pt-5">
              ...
              {isAuthenticated ? (
                <div className="flex gap-3">
                  <button
                    ...
                    aria-label={inCart ? "Ya está en el carrito" : "Agregar al carrito de consulta"}
                    disabled={inCart}
                  ...
                  >
                    <svg viewBox="0 0 24 24" fill={isFav ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.8" className="w-5 h-5" aria-hidden="true">
              ...
              <p className="text-xs text-[#6e6e73] text-center">
                {isAuthenticated
                  ...
                  : <>
                      <Link to="/login" className="underline underline-offset-2 hover:text-[#1d1d1f] transition-colors">Iniciá sesión</Link>
            ...
            {/* Badges de confianza */}
            <div className="grid grid-cols-3 gap-3 border-t border-black/[0.06] pt-5">
              {[
                { icon: "🛡", label: "Garantía oficial" },
                { icon: "📦", label: "Envío a todo ARG" },
                { icon: "✓", label: "Stock confirmado" },
              ].map((b) => (
                <div key={b.label} className="flex flex-col items-center gap-1 text-center bg-white rounded-xl p-3 border border-black/[0.05]">
                  <span className="text-lg" aria-hidden="true">{b.icon}</span>
                  <span className="text-[10px] text-[#6e6e73] leading-tight">{b.label}</span>
                </div>
              ))}
            </div>
          </div>
